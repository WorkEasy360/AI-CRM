...
import type { CustomData, Deal, DealInput, Pipeline } from "@/lib/api/crm-types";
...
function defaultsFor(deal: Deal | null, pipelines: Pipeline[], defaultPipelineId: string | undefined, baseCurrency: string): DealFormValues {
  ...
  return {
    ...
    company_id: deal?.company?.id ?? NONE,
    primary_contact_id: deal?.primary_contact?.id ?? NONE,
...
/**
 * Create or edit a deal. Pipeline and stage are chosen on create only (the API rejects them on
 * update; use the stage move flow instead). Custom fields render from the deal definitions.
 */
export function DealFormDialog({
  ...
  const form = useForm<DealFormValues>({
    ...
    defaultValues: defaultsFor(existing, pipelines, defaultPipelineId, baseCurrency),
  });

  React.useEffect(() => {
    if (open) {
      form.reset(defaultsFor(existing, pipelines, defaultPipelineId, baseCurrency));
      setCustomData(existing?.custom_data ?? {});
      setFieldErrors({});
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, existing, pipelines, defaultPipelineId, baseCurrency]);
      ...
      <DialogContent className="max-h-[calc(100vh-2rem)] max-w-2xl overflow-y-auto">
        <form onSubmit={onSubmit} className="grid gap-4" noValidate>
              ...
              {(field) => (
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    {existing?.company && !companyOptions.some((c) => c.id === existing.company?.id) ? (
                      <SelectItem value={existing.company.id}>{existing.company.name}</SelectItem>
                    ) : null}
              ...
              {(field) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  ...
                  <SelectContent>
                    ...
                    {existing?.primary_contact && !contactOptions.some((c) => c.id === existing.primary_contact?.id) ? (
                      <SelectItem value={existing.primary_contact.id}>{existing.primary_contact.name}</SelectItem>
                    ) : null}
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <FormField control={form.control} name="probability" label="Probability (%)" serverError={fieldErrors.probability} description={probabilityLocked ? "Fixed for closed deals." : undefined}>
              {(field) => (
                <Input {...field} inputMode="numeric" placeholder="Stage default" value={field.value} onChange={(e) => field.onChange(e.target.value)} disabled={probabilityLocked} />
              )}
            </FormField>
          </div>
