...
- **Network**: public subnets only for ALB/NAT; tasks and data in private subnets; one security group per workload; `api/workers/beat/migrate→db:5432`, `→redis:6379` by group reference, nothing inbound to data tiers from the internet; egress is per workload and port-scoped (TCP 443 through NAT for api/web/workers, TCP 587 for worker-critical SMTP, none for beat/migrate; AWS APIs via VPC endpoints). Package registries are only reached during image builds.
