...
| Flow | Design |
|---|---|
| Sign-up | Name (optional, sanitized) + email + password (Argon2id via Django's `Argon2PasswordHasher` as default hasher). Creates `user` (unverified). Verification email with single-use, hashed, 24 h token. Login is blocked until verified. Response is generic whether the email exists or not. On verification the server creates the user's personal workspace (`organization` + `Owner` membership + default sales pipeline) in one transaction, idempotently (`accounts.services.ensure_personal_organization`); login and `POST /session/bootstrap/` repeat the same idempotent step as fallbacks, so no client-side organization setup exists. |
...
| Invitation | Admin invites by name, email, role and optional team. Hashed token, 7-day expiry, resend rotates the token. Acceptance creates the membership (existing user) or, from the link, the user with their own password + membership in one locked transaction (new user). Invitation-based accounts verify email implicitly through the token. See `user-management.md`. |
